...
export function readFile(localPath) {
  return new Promise((resolve) => {
    fs.readFile(path.join(process.cwd(), localPath), 'utf8', (err, data) =>
      resolve(data)
    );
  });
}
...
export function readDirectory(localPath) {
  return new Promise((resolve) => {
    fs.readdir(path.join(process.cwd(), localPath), (err, data) =>
      resolve(data)
    );
  });
}
...
export const getBlogPostList = React.cache(() => {
  ...
  return fs.readdirSync(localPath).map((filename) => {
