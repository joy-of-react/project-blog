...
To understand why this works, it's worth remembering our new model for division: we're trying to divide `timeElapsed` into 3 equally-sized groups, without any fractional or decimal values.

The remainder will always be either 0, 1, or 2. It will never be 3+, because if there _was_ 3 left, we could fit 1 more in each group!

And so, this trick allows us to essentially create “circular” arrays, allowing us to cycle through the values. It's an _incredibly_ handy trick, something I've used dozens of times in the years since I first learned of it.
